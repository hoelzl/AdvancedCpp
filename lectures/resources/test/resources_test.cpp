...
#if 0
#define MY_CHECK(expr, check)                                                          \
    ((check) ? [&] { CHECK(expr); }() : [&] { static_assert(expr); }())
...
test_fake_raii_container(bool check = false, bool disable_self_checks = false)
{
    ...
    auto his_container = res::FakeRaiiContainer<int>{nullptr};
...
static_assert(test_fake_raii_container(false));

TEST_CASE("Fake RAII.") { test_fake_raii_container(true, false); }

TEST_CASE("Check static properties.")
{
    STATIC_CHECK(std::is_move_assignable_v<res::FakeRaiiContainer<int>>);
}
